...
export const projectMemberEnum = pgEnum("project_member", [
  "member",
  "admin",
  "creator",
])
...
export const ProjectsTable = pgTable("projects", {
  ...
  createdAt: timestamp("createdAt", { mode: "date", withTimezone: true })
    .notNull()
    .defaultNow(),
  tier: projectTierEnum("tier").notNull().default("free"),
...
export const ProjectMembersTable = pgTable(
  "project_members",
  {
    ...
    userId: text("user_id")
      .notNull()
      .references(() => UsersTable.id, { onDelete: "cascade" }),
    role: projectMemberEnum("role").notNull().default("member"),
...
export const ProjectTasksTable = pgTable("project_tasks", {
  ...
  status: projectTaskEnum("status").default("pending").notNull(),
  priority: projectTaskPriorityEnum("priority").default("low").notNull(),
  createdAt: timestamp("createdAt", { mode: "date", withTimezone: true })
    .notNull()
    .defaultNow(),
...
export const ProjectIssuesTable = pgTable("project_issues", {
  ...
  taskIssueId: text("task_issue_id").references(() => ProjectTasksTable.id, {
    onDelete: "cascade",
  }),
...
export const ProjectTaskRelations = relations(
  ProjectTasksTable,
  ({ one, many }) => ({
    project: one(ProjectsTable, {
...
export const ProjectTaskAssignRelations = relations(
...
export const ProjectIssueRelations = relations(
  ProjectIssuesTable,
  ({ one, many }) => ({
    project: one(ProjectsTable, {
...
export const ProjectIssueRepliesRelations = relations(
