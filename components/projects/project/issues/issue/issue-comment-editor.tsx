...
export default function IssueCommentEditor({
    ...
    const handleIssueDescriptionChange = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        ...
        try {
            ...
            const response = await updateIssueDescription(issueId, validatedFields.data.comment, projectId)
            ...
            <DialogContent className="sm:max-w-[600px] md:max-w-[800px]">
                ...
                <form onSubmit={handleIssueDescriptionChange}>
                    ...
                    <DialogFooter className="mt-4">
                        ...
                        >
                            {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Mentés"}
                        </Button>
                    </DialogFooter>
                </form>
            </DialogContent>
