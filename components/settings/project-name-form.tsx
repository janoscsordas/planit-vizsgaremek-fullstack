...
const ProjectNameForm = ({
  ...
  const getRemainingDays = (): number | null => {
    ...
    return (
      COOLDOWN_DAYS -
      differenceInDays(new Date(), new Date(projectData.projectNameChangedAt))
    )
  }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!validatedData.success) {
        throw new Error(
          validatedData.error.errors[0]?.message || "Érvénytelen projektnév!"
        )
      }
      ...
      <div className="space-y-2">
        ...
        {formState.disabled && remainingDays ? (
          <p className="text-xs text-red-500">
            {remainingDays} nap múlva módosíthatod újra a projekt nevét.
          </p>
        ) : (
