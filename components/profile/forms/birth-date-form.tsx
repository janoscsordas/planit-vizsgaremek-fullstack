...
export default function BirthDateForm({
  ...
  const handleDateSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (Number(birthDate.split("-")[0]) > new Date().getFullYear()) {
      toast.error("A megadott születési dátum nem megengedett!", {
        duration: 3000,
      })
      return
    }
    ...
    if (!validatedBirthDate.success) {
      toast.error("Hibás formátumú születési dátumot adtál meg!", {
        duration: 3000,
      })
      return
    }
  ...
  return (
    <section className="p-6 mt-6 border rounded-md border-muted">
      ...
      {birthDateState ? (
        <>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Ez a beállított születési dátumod!
          </p>
        </>
      ) : (
        <form onSubmit={handleDateSubmit}>
          ...
          />
          <p className="mt-1 text-xs text-muted-foreground">
            A születési dátumod csak egyszer tudod megadni!
          </p>
          <Button
            type="submit"
            className="mt-4 bg-emerald hover:bg-emerald-hover"
          >
            Mentés
          </Button>
        </form>
