...
export default function Projects({ userSession }: { userSession: User }) {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
        ...
        return { ownedProjects, memberProjects } as { ownedProjects: Project[], memberProjects: Project[] }
    },
    staleTime: 60 * 1000,
  ...
  const filteredProjects = useMemo(() => {
    const ownedProjects = data?.ownedProjects?.filter((project: Project) =>
      project.name.toLowerCase().includes(searchTerm.toLowerCase()) &&
      (statusFilter === 'all' || project.status === statusFilter)
    ) || [];
    
    const memberProjects = data?.memberProjects?.filter((project: Project) =>
      project.name.toLowerCase().includes(searchTerm.toLowerCase()) &&
      (statusFilter === 'all' || project.status === statusFilter)
    ) || [];
  ...
  return (
    <div className='space-y-6 w-[95%] mx-auto'>
      ...
      <div className="space-y-6">
        ...
        {isLoading ? <Spinner /> : (
          <>
            <ProjectList title={userSession?.name + ' Projektjei'} projects={filteredProjects.ownedProjects as Project[]} />
            <ProjectList title="Tagja vagy a következő projekteknek:" projects={filteredProjects.memberProjects as Project[]} />
          </>
        )}
      </div>
    </div>
