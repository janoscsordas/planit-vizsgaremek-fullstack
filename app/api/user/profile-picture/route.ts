...
export async function POST(request: Request) {
    ...
    if (imageChangedAt.imageChangedAt && differenceInDays(new Date(), imageChangedAt.imageChangedAt) < 90) {
        return NextResponse.json({ error: `Legközelebb csak ${90 - differenceInDays(new Date(), imageChangedAt.imageChangedAt)} nap múlva módosíthatod a profilképet!` })
    }

    const { fileUrl, uploadedBy } = await request.json()

    if (!fileUrl || !uploadedBy) {
        return NextResponse.json({ error: "Hiba történt a profilkép változtatása közben!" }, { status: 400 })
    }
