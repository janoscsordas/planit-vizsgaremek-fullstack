        ...
        <Dialog>
            ...
            <DialogContent className="w-full sm:max-w-[425px]">
                ...
                >
                    ...
                    {state?.errors?.taskName && state.errors.taskName.map((error: string, index) => <p key={index} className="text-sm text-red-500">{error}</p>)}
                    <div className="grid grid-cols-4 items-center gap-4 mb-3">
                        ...
                        <div className="relative col-span-3">
                            ...
                            />
                            <span className="absolute -bottom-6 right-0 text-xs text-muted-foreground">Még {256 - descriptionLength} karaktert írhatsz</span>
                        </div>
                    </div>
                    {state?.errors?.taskDescription && state.errors.taskDescription.map((error: string, index) => <p key={index} className="text-sm text-red-500">{error}</p>)}
                    ...
                    {state?.errors?.priority && state.errors.priority.map((error: string, index) => <p key={index} className="text-sm text-red-500">{error}</p>)}
                    <div className="grid grid-cols-4 items-center gap-4 my-1">
                        <Label htmlFor="status" className="text-left">
                            Státusz:
                        </Label>
                        <Badge variant={"secondary"} className={cn("w-max", 
                            status === "in progress" && 
                            ...
                            {status === "pending" ? "Elvégzendő" : status === "in progress" ? "Folyamatban" : "Befejezett"}
                        </Badge>
