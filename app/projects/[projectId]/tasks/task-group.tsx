...
const TaskGroup = React.memo(({ title, count, tasks, projectId }: TaskGroupProps) => {
        ...
        if (tasks.length === 0) {
            return (
                <div className="group flex items-center gap-2 rounded-md px-2 py-1 hover:bg-muted/50 transition-colors duration-200">
                    ...
                    <span className="text-sm text-muted-foreground">Még nem készítettél ide feladat.</span>
                </div>
            );
        }
    ...
    return (
        <Collapsible open={isOpen} onOpenChange={setIsOpen} className="w-full">
            <div className="flex items-center py-2 w-full">
                ...
                <TaskCreate status={title.toLowerCase() as "pending" | "in progress" | "finished"} projectId={projectId}>
