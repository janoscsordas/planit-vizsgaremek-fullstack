...
async function getPaginatedIssues(
  ...
  const issues = await db.query.ProjectIssuesTable.findMany({
    ...
    orderBy: (ProjectIssuesTable, { desc }) =>
      desc(ProjectIssuesTable.openedAt),
    limit: limit,
    offset: offset,
  })
...
export default async function Page({
  ...
  const totalCountOfIssues = {
    ...
    closed: projectDataWithCounts.issues.filter((issue) => !issue.isOpen)
      .length,
  }
  ...
  const issueConditions = [
    ...
    ...(status !== null && statusFilter !== null
      ? [eq(ProjectIssuesTable.isOpen, statusFilter)]
      : []), // Filter by status
    ...(searchQuery
    ...
    <>
      ...
      <main className="px-6 py-2">
        <div>
          <IssuesTable
            issues={issues}
            projectId={projectDataWithCounts.id}
            countOfIssues={totalCountOfIssues}
