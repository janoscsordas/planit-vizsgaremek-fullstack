...
export async function updateTaskPriority(
  ...
  const validatedFields = updateTaskPrioritySchema.safeParse({
    taskId,
    priority,
  })
  ...
  try {
    await db
      ...
      .set({ priority: validatedFields.data.priority })
      .where(eq(ProjectTasksTable.id, validatedFields.data.taskId))
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
  } catch (error) {
    return {
      success: false,
...
export async function updateTaskDescription(
  ...
  const validatedFields = updateTaskDescriptionSchema.safeParse({
    taskId,
    description,
  })
  ...
  try {
    await db
      .update(ProjectTasksTable)
      .set({ taskDescription: validatedFields.data.description })
      .where(eq(ProjectTasksTable.id, validatedFields.data.taskId))
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
  } catch (error) {
    return {
...
export async function updateTaskName(
  ...
  try {
    await db
      .update(ProjectTasksTable)
      .set({ taskName: validatedFields.data.name })
      .where(eq(ProjectTasksTable.id, validatedFields.data.taskId))
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
  } catch (error) {
    return {
...
export async function changeTaskStatus(
  ...
  try {
    await db
      .update(ProjectTasksTable)
      .set({ status: validatedFields.data.status })
      .where(eq(ProjectTasksTable.id, validatedFields.data.taskId))
  } catch (error) {
    return {
      success: false,
      message:
        error instanceof Error
          ? error.message
          : "Hiba történt a feladat státusz módosítása közben!",
    }
  }
...
export async function removeUserFromTaskAssignsAction(
  ...
  try {
    await db
      ...
      .where(
        and(
          eq(ProjectTaskAssignsTable.taskId, taskId),
          eq(ProjectTaskAssignsTable.userId, userId)
        )
      )
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
  } catch (error) {
  ...
  return {
    success: true,
    message: "A tag sikeresen eltávolítva a feladat kiosztásból!",
  }
}
